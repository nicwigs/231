...
def get_transposability_zero(num):
    ...
    for lier in range(2,10):                  #range of multipliers
        ...
        while len(num_str) != len(str(ple)):  #add zeros in front until same...
            num_str = "0" + num_str           #...length as multiple
        if is_transpose(num_str,str(ple)):#see if transpoeses num and multiple
            print("{:>10s} {} {} {} {:<10d}".\
...
def process_file(): 
    ...
    calls get_transposability and get_transposability_zero for all values 
        depending if they need a 0 or not appended to the front
    from start to end  
    return: None
    """
    start = 0  #range start value recieved from file
    end = 0    #range end value recieved from file
    lier = 0   #multiplier for transposability
    ple = 0    #multiple for transposability
    t0 = False #bool see if one of the values needs leading 0s to be tested
                   #for transposability
    ...
    for i in range(start,end+1):     #through given range(Inclusive via Piazza)
        t0 = False                   #initialize bool each time through loop
        for lier in range(2,10):     #go through each possible multiple
            ple = lier * i
            if len(str(i)) != len(str(ple)):#if one of the 2-9 multiples has 
                t0 = True                   #differing lengths note it
        if t0:#if one combination needs leading zero use the zero function to 
                  #go through all multiples & check transposability
            get_transposability_zero(i)
        else: #if they are all the same length aka no leading 0 needed send to
            get_transposability(i) #normal function to check transposability
#-------------------------------------------------------------------------
##############################################################
...
#the program body following calls process_file() which calls other functions 
    ...
    #function header description, a basic summary follows
#########################################################################
